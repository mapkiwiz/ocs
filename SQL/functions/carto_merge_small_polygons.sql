...
--
CREATE OR REPLACE FUNCTION carto_merge_small_polygons(source_table text, destination_table text, type_attr text default 'type', max_area double precision default 2500, target_schema text default 'public')
RETURNS integer
AS
$func$
DECLARE
...
BEGIN
    ...
    LOOP
        ...
        LOOP
            len := st_length(safe_intersection(feature.geom, p, 0.001));
            IF len > max_len THEN
                max_len := len;
                target := feature;
                matched := true;
        ...
        END LOOP;
        IF matched THEN
            u := safe_union(p, target.geom, 0.001);
