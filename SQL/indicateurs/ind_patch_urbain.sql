...
BEGIN
    ...
    LOOP
        ...
        patches AS (
            ...
            FROM  carto.surface_urbanisee a
...
CREATE OR REPLACE FUNCTION ind.ind_patch_urbain(query_dept text)
RETURNS TABLE (cid int, num_obj int)
AS
$func$
DECLARE
...
CREATE TABLE ind.patch_urbain_cantal AS
SELECT * FROM ind.ind_patch_urbain('CANTAL');

ALTER TABLE ind.patch_urbain_cantal
ADD COLUMN geom geometry(Polygon, 2154);

UPDATE ind.patch_urbain_cantal
SET geom = g.geom
FROM ind.grid_500m g
WHERE patch_urbain_cantal.cid = g.gid;