...
CREATE OR REPLACE FUNCTION ind.ind_foret_frag(query_dept text)
RETURNS TABLE (cid int, num_obj int)
AS
$func$
DECLARE
...
CREATE TABLE ind.foret_frag_cantal AS
SELECT * FROM ind.ind_foret_frag('CANTAL');

ALTER TABLE ind.foret_frag_cantal
ADD COLUMN geom geometry(Polygon, 2154);

UPDATE ind.foret_frag_cantal
SET geom = g.geom
FROM ind.grid_500m g
WHERE foret_frag_cantal.cid = g.gid;