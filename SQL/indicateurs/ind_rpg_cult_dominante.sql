...
RETURNS TABLE (cid int, surf_rank int, cult_maj int, surf_ha double precision)
AS
$func$
DECLARE
...
BEGIN
    ...
    LOOP
        ...
        WITH
        clc AS (
            SELECT a.cult_maj, sum(st_area(st_intersection(a.geom, b.geom))) / 1e4 as surf_ha
            FROM ref.rpg_2012 a INNER JOIN ind.grid_500m b ON st_intersects(a.geom, b.geom)
            WHERE b.gid = cell_id
            GROUP BY a.cult_maj
        ),
        surf AS (
...
CREATE OR REPLACE FUNCTION ind.ind_rpg_cult_dominante(query_dept text)
RETURNS TABLE (cid int, surf_rank int, cult_maj int, surf_ha double precision)
AS
$func$
DECLARE
...
CREATE TABLE ind.rpg_cult_dominante_cantal AS
SELECT * FROM ind.ind_rpg_cult_dominante('CANTAL');