...
CREATE OR REPLACE FUNCTION ind.ind_patch_foret(query_dept text)
RETURNS TABLE (cid int, num_obj int)
AS
$func$
DECLARE
...
CREATE TABLE ind.patch_foret_cantal AS
SELECT * FROM ind.ind_patch_foret('CANTAL');

ALTER TABLE ind.patch_foret_cantal
ADD COLUMN geom geometry(Polygon, 2154);

UPDATE ind.patch_foret_cantal
SET geom = g.geom
FROM ind.grid_500m g
WHERE patch_foret_cantal.cid = g.gid;