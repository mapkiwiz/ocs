CREATE OR REPLACE FUNCTION ind.ind_ecotone_foret_longueur(cell_10km bigint)
...
CREATE OR REPLACE FUNCTION ind.ind_ecotone_foret_longueur(query_dept text)
RETURNS TABLE (cid int, length_m double precision)
AS
$func$
DECLARE
...
BEGIN
    ...
    LOOP
        ...
        RETURN QUERY
        SELECT * FROM ind.ind_ecotone_foret_longueur(cell_10km);
...
CREATE OR REPLACE FUNCTION ind.ind_ecotone_foret_longueur()
RETURNS TABLE (cid int, length_m double precision)
AS
$func$
DECLARE
...
CREATE TABLE ind_aura.ecotone_foret_longueur AS
SELECT * FROM ind.ind_ecotone_foret_longueur();